...
export default function Header() {
  ...
  return (
    <header className="bg-white shadow-lg sticky top-0 z-50" onKeyDown={handleKeyDown}>
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <Link 
            href="/" 
            className="flex items-center space-x-3 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 rounded-lg transition-all duration-200"
            aria-label="Advogados Sorocaba - Página inicial"
            onClick={closeAllMenus}
          >
            <div className="w-12 h-12 bg-blue-700 rounded-lg flex items-center justify-center shadow-md">
              <span className="text-white font-bold text-lg">AS</span>
            </div>
            <div className="hidden sm:block">
              <h1 className="text-xl font-bold text-gray-900 leading-tight">Advogados Sorocaba</h1>
              <p className="text-sm text-gray-600 leading-tight">Escritório de Advocacia</p>
            </div>
          </Link>
                        ...
                        <div className="py-2">
                          {item.items.map((subItem) => (
                            <Link
                              key={subItem.name}
                              href={subItem.href}
                              className="block px-4 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-700 transition-colors duration-200 group focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 focus:bg-blue-50 rounded-lg mx-2"
                              onClick={closeAllMenus}
                            >
          ...
          <div className="hidden lg:flex items-center space-x-6">
            ...
            >
              ...
              <span className="font-medium hidden xl:inline">{CONTACT_INFO.phone}</span>
            </a>
        ...
        {isMobileMenuOpen && (
          <div
            id="mobile-menu"
            ref={mobileMenuRef}
            className="md:hidden absolute top-20 left-0 right-0 bg-white border-t border-gray-200 shadow-2xl z-40 animate-in fade-in-0 slide-in-from-top-2 duration-200"
          >
                  ...
                  if (item.type === 'dropdown') {
                    return (
                      <div key={item.name} className="space-y-2">
                        <h3 className="text-lg font-semibold text-gray-900 px-3 py-2 border-l-4 border-blue-500">
                          {item.name}
                        </h3>
                        <div className="space-y-1 pl-4 border-l-2 border-blue-200 ml-3">
                          {item.items.map((subItem) => (
                            <Link
                              key={subItem.name}
                              href={subItem.href}
